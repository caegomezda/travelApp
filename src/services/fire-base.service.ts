...
import { User } from '../app/aInterfaces/fire-base-interface';
...
export class FireBaseService {
  ...
  async fetchUserInfo2Api(credential,urlType): Promise<any>{
    ...
    let json = {}
    json = JSON.stringify(json);
    let result =  await this.http.get(`${apiUrl}`, json).pipe(map( async data => await data)).toPromise();
  ...
  async getData(dataJson){
    console.log('ENTRO A GET DATA________________________________________________');
    let data = [];
  ...
  async getAccountData(){
    ...
    console.log('result______________________________________another',await result);
